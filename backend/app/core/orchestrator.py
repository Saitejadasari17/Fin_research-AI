...
class DynamicOrchestrator:
    """
    Investment research orchestrator for company analysis, valuation, and thesis synthesis.
    """
    ...
    def run_investigation(
        ...
        # Cyclical FCF Normalization:
        # If trailing FCF is negative or zero (e.g. semiconductor cyclical trough / CapEx burst),
        # fallback to operating income * 0.70 or net income * 0.80 if positive.
        if fcf <= 0:
            if op_income > 0:
                fcf = op_income * 0.70
            elif net_income > 0:
        ...
        # ====================================================
        # GROWTH ASSUMPTION (INTELLIGENT SECTOR & FUNDAMENTAL DETERMINATION)
        # ====================================================

        growth_value = (
            market_data.get("high_growth_rate")
            or financial_data.get("high_growth_rate")
        )
        ...
        if growth_value is not None:
            ...
            rev_cagr = financial_data.get("revenue_cagr_3yr")
            if rev_cagr is not None and 5.0 <= rev_cagr <= 50.0:
                growth = float(rev_cagr) / 100.0
            else:
                sec = (context.sector or "").lower()
                ind = (context.industry or "").lower()
                sym = (context.ticker or "").upper()

                if sym in ["NVDA", "AMD", "AVGO", "TSM", "ARM", "ASML"]:
                    growth = 0.35
                elif sym == "MU" or "memory" in ind:
                    growth = 0.28
                elif "semiconductor" in ind or "semiconductor" in sec:
                    growth = 0.32
                elif "technology" in sec or "communication" in sec or sym in ["MSFT", "META", "GOOGL", "AMZN", "AAPL", "TSLA"]:
                    growth = 0.22
                elif self._is_indian_company(context):
                    growth = 0.15
                else:
                    growth = 0.12

        growth = max(0.05, min(0.45, growth))
        ...
        wacc_value = financial_data.get("wacc")

        if wacc_value is None:
            sym = (context.ticker or "").upper()
            if is_india:
                wacc = 0.115
            elif sym in ["NVDA", "MSFT", "GOOGL", "META", "AAPL", "AMZN"]:
                wacc = 0.085
            else:
                wacc = 0.090
        else:
            wacc = float(wacc_value)

        terminal_growth_value = financial_data.get("terminal_growth_rate")

        if terminal_growth_value is None:
            terminal_growth = 0.04 if is_india else 0.025
        else:
        ...
        # ====================================================
        # RECOMMENDATION (MULTI-FACTOR DEBATE & VALUATION MATRIX)
        # ====================================================

        overall_conf = confidence_breakdown.get("overall", 0.0)
        upside_pct = dcf_result.upside_downside_percent
        mc_prob = monte_carlo_result.probability_undervalued_percent if monte_carlo_result else 50.0
        bull_won = ("bull case" in (debate.judge_verdict or "").lower() or len(debate.bull_arguments) >= len(debate.bear_arguments))

        if (
            debate.thesis_survival_status == "Insufficient Evidence"
            or overall_conf < 50.0
        ):
            recommendation = "INSUFFICIENT EVIDENCE"
        elif valuation_status == "Undervalued" or upside_pct >= 15.0:
            recommendation = "BUY"
        elif valuation_status == "Fairly Valued" or upside_pct >= -15.0:
            if bull_won and mc_prob >= 40.0:
                recommendation = "BUY"
            else:
                recommendation = "HOLD"
        elif valuation_status == "Overvalued":
            if bull_won and mc_prob >= 50.0 and upside_pct >= -25.0:
                recommendation = "HOLD"
            elif not bull_won or mc_prob < 35.0 or upside_pct < -30.0:
                recommendation = "SELL"
            else:
                recommendation = "HOLD"
        else:
