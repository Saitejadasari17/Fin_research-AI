...
11. Perform deterministic zero-LLM financial calculations (DCF, WACC, Ratios)
12. Run 5,000-iteration Monte Carlo probabilistic simulations
13. Challenge its own investment thesis via adversarial Bull vs. Bear debate
14. Estimate confidence and uncertainty
15. Produce an evidence-backed investment research memo
...
| Interview Question | Senior-Level Architectural Answer |
| :--- | :--- |
| **Why use an agent instead of RAG?** | *"RAG is a single retrieve-and-generate pass. Investment research is an open-ended investigation where the next step depends on previous findings. Our system dynamically decides what to investigate next, selects tools, detects contradictions, and tests thesis resilience."* |
| **How do you prevent math hallucinations?** | *"We strictly separate reasoning from calculation. LLMs handle qualitative decision-making and claim parsing; deterministic Python code (`numpy`/`pandas`) executes all DCF models, WACC calculations, and Monte Carlo simulations."* |
| **Where does the agent make actual decisions?** | *"In `decide_next_action(state)`. At each loop iteration, the agent inspects accumulated findings, open research questions, and confidence levels to pick the next tool and arguments or declare research complete."* |
| **How do you handle prompt injection in web sources?** | *"All retrieved external text passes through `SecurityGatekeeper.sanitize_retrieved_text()` and is wrapped in XML data isolation tags (`<untrusted_retrieved_data>`), enforcing a strict `DATA != INSTRUCTION` policy."* |
